      ...
      <div class="modal-content">
        <div class="modal-header" id="kt_modal_add">
          <template v-if="Modal && dataModal.id">
            <h2 class="fw-bold">Edit Users</h2>
          </template>
          <template v-else>
            <h2 class="fw-bold">Tambah Users</h2>
          </template>
          <div
        ...
        <template v-if="Modal && dataModal.id">
          <AddData @on-save="submit()" :data="dataModal"></AddData>
        </template>
        <template v-else>
          <AddData @on-save="submit()" :data="dataModal"></AddData>
        </template>
...
export default defineComponent({
  ...
  setup() {
    ...
    const clearForm = () => {
      ...
      dataModal.value.email = "";
      dataModal.value.role_id = "";
    }
  ...
  computed: {
        selectItem(): any {
            const except = ["log-info", "actions","role"];
            return [
                ...this.tableHeader.filter(
                    (col) => !except.find((e) => e === col.columnLabel)
                ),
                { columnName: "Role", columnLabel: "role.nama" },
            ];
        },
    },
});
