  ...
  <div class="card">
    ...
    <div class="card-body pt-0">
      <Datatable
        ...
        checkbox-label="id"
      >
        <template v-slot:kode="{ row: data }">
          {{ data.kode }}
        </template>
        <template v-slot:tanggal="{ row: data }">
          {{ data.tanggal }}
        </template>
        <template v-slot:merchant_id="{ row: data }">
          {{ data.merchant.nama }}
        </template>
        ...
        <template v-slot:actions="{ row: data }">
          ...
          <ul class="dropdown-menu">
            <li>
              ...
              data-bs-target="#kt_modal_add">Lihat Invoice</a>
            </li>
          </ul>
        </div>
        </template>
    ...
    <div class="modal-dialog modal-dialog-centered mw-650px">
      <div class="modal-content">
        ...
        <template v-if="Modal && dataModal.id">
          <ViewData :data="dataModal"></ViewData>
        </template>
      </div>
    </div>
  </div>
...
export default defineComponent({
  ...
  setup() {
    ...
    const getData =(props) => {
        ...
        let url = "order?table=true";
        url = url + "&page=" + page;
        url = url + "&limit=" + perpage;
        url = url + "&order=tanggal:asc";
        url = url + "&search=merchant_id:"+JwtService.getUser().merchant.id+"|";
    ...
    const viewModal = (id: string) => {
      dataModal.value.merchant_id = null
      dataModal.value.customer_id = null
      dataModal.value.kode = ''
      dataModal.value.tanggal = ''
  ...
  computed: {
        selectItem(): any {
            const except = ["log-info", "actions","customer_id","merchant_id"];
            return [
                ...this.tableHeader.filter(
                    (col) => !except.find((e) => e === col.columnLabel)
                ),
                { columnName: "Customer", columnLabel: "customer.name" },
                { columnName: "Merchant", columnLabel: "merchant.nama" },
            ];
        },
    },
});
