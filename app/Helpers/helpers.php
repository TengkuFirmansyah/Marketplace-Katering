...
use App\Models\PembayaranMahasiswaDetail;
use App\Models\Pendaftaran;
use App\Models\Whatsapp;
use App\Models\WhatsAppChatDetails;
...
function H_paginate($items, $endpointName, $totalData, $perPage = 5, $paramsUrl = "", $page = null){
...
function H_dropColumnIfExists($myTable, $column)
{
    if (is_array($column)) {
        $columns = [];
        foreach($column as $r) 
            if (Schema::hasColumn($myTable, $r)) $columns[] = $r;

        if (count($columns))
            Schema::table($myTable, function (Blueprint $table) use ($columns) {
                foreach($columns as $r) $table->dropColumn($r);
            });
    } else if(Schema::hasColumn($myTable, $column)) {
...
function H_extractKeyRelation($key) {
    ...
    $ex = explode('.', $key);
    if (
        count($ex) > 1 && // check validation
        strlen($ex[0]) > 0 && strlen(end($ex)) > 0  // check value
    ) {
        $importantCheck = explode('!', end($ex));
...
function H_arrPerweek($start, $end) {
    ...
    while ($start->timestamp <= $end->timestamp) {
        ...
        if ($start->dayOfWeek !== 5) $start->addDay(5 - $start->dayOfWeek);
        else $start->addWeek(1);
    }

    return $arr;
}

function H_CodePendaftaran($year, $periode, $jalur, $program, $model) {
    $code_jalur = MasterJalur::where('id', $jalur)->first();
    $code_program = MasterProgram::where('id', $program)->first();
    $data_periode = MasterPeriode::where('id', $periode)->first();
    if($data_periode->semester == 'Gasal'){
        $code_periode = 1;
    } else {
        $code_periode = 2;
    }
    $tahun = substr($data_periode->periode_awal,2);

    $check = $model::where('periode_id','=', $periode)
                    ->where('program_id','=', $program)
                    ->where('jalur_id','=', $jalur)
                    ->whereNotNull('code')
                    ->orderBy('code', 'desc');
    if ($check->first() !== null) {
        $codelast = $check->first()->code;
        $tambah = 1;
        $num = (int) substr($codelast, -4);
        $nex = sprintf("%04s", $num+$tambah);
        $code = $tahun.$code_periode.$code_jalur->code.$code_program->code.$nex;

        while (Pendaftaran::where('code', $code)->exists()) {
            $tambah++;
            $num = (int) substr($codelast, -4);
            $nex = sprintf("%04s", $num+$tambah);
            $code = $tahun.$code_periode.$code_jalur->code.$code_program->code.$nex;
        ...
        // $code = $tahun.$code_periode.$code_jalur->code.$code_program->code.'0001';
    }
    return $code;
}

function H_Informasi($parent, $module, $note, $auth, $to) {
    ...
    $save->to = $to;
    $save->is_read = 0;
    $save->created_by = $auth;
    $save->save();
}

function notifikasiWhatsapp($data, $jenjang, $penerima, $bagian, $posisi, $va = "")
{
    $notif = MasterNotifikasi::select("*")
                ->where("jenjang_pendaftaran_id", $jenjang)
                ->where("bagian", $bagian)
                ->where("posisi", $posisi)
                ->first();
    if($notif) {
        $text_wa = $notif->notif;
        if(strpos($text_wa, '$nama_camaba')){
            $text_wa = str_replace('$nama_camaba', $data->mahasiswa->nama, $text_wa);
        }
        if(strpos($text_wa, '$code_pendaftaran')){
            $text_wa = str_replace('$code_pendaftaran', $data->code, $text_wa);
        }
        if(strpos($text_wa, '$jalur_pendaftaran')){
            $text_wa = str_replace('$jalur_pendaftaran', $data->jalur->nama, $text_wa);
        }
        if(strpos($text_wa, '$prodi_pilihan') && isset($data->details)){
            $prodi = "";
            foreach ($data->details as $det){
                $prodi = $prodi." ".$det->prodi->nama.",";
            }
            $text_wa = str_replace('$prodi_pilihan', $prodi, $text_wa);
        }
        if(strpos($text_wa, '$va') && $va != ""){
            $text_wa = str_replace('$va', $va, $text_wa);
        }
        if(strpos($text_wa, '$nominal') && $va != ""){
            $nominal = PembayaranMahasiswaDetail::select("*")->where('va', $va)->first();
            $text_wa = str_replace('$nominal', number_format($nominal->nominal, 2), $text_wa);
        }
        return sendWa($penerima, $text_wa, $bagian);
    }
    return true;
}

function sendWa($nowa, $text_wa, $bagian = null)
{
    $wa = Whatsapp::where('status','=','connected')->where("bagian", $bagian)->first();
    if ($wa && $nowa != null) {
        $find = Http::withHeaders([
            'x-api-key' => 'PANCASILAM4JU'
            ])->get('http://wa-engine.univpancasila.ac.id/session/status/'.$wa->number);
        $find = json_decode($find);
        if(isset($find->message) && $find->message == "session_connected"){
            $number = preg_replace( '/[^0-9]/', '', $nowa );
            if (substr($number, 0,2) != '62') {
                $getnumber = substr($number, 1);
                $regional = 62;
                if(substr($number, 0,1) == 0 && substr($number, 1,1) == 8){
            ...
            $response = Http::withHeaders([
                'x-api-key' => 'PANCASILAM4JU'
            ])->post('http://wa-engine.univpancasila.ac.id/client/sendMessage/'.$wa->number, [
                // "chatId" => "6282370406655@c.us",
                "chatId" => $format_number."@c.us",
                "content" => $text_wa."\n\n- ".$wa->name,
                "contentType" => "string",
            ]);
            return $response;
...
function WhatsappFacebookButton($nomor) {
    $number = preg_replace( '/[^0-9]/', '', $nomor );
    if (substr($number, 0,2) != '62') {
        $no_wa = $number;
        $regional = 62;
        if(substr($number, 0,1) == 0 && substr($number, 1,1) == 8){
        ...
        $format_number = $number;
        $regional = 0;
        $no_wa = $regional.substr($nomor,2);
    }
    $pendaftaran = Pendaftaran::select("*","profile_mahasiswa.nama")
                        ->with("jalur")
                        ->join("profile_mahasiswa","profile_mahasiswa.id","=","pendaftaran.mahasiswa_id")
                        ->where("profile_mahasiswa.no_hp", $no_wa)
                        ->where("pendaftaran.status", 1)
                        ->first();
    $whatsapp = Whatsapp::select("*")->where("bagian","=","Registrasi")->first();
    $tekswa = "";
    if($whatsapp){
        $tekswa = "\n\nInfo lebih lanjut bisa menghubungi Admin Registrasi di Nomor berikut :\n".$whatsapp->number;
    }
    if($pendaftaran){
        $data = [
            ...
            "type" => "text",
            "text" => [
                "body" => "Data Pendaftaran anda sebagai berikut : \n\n*Nama Calon Mahasiswa:* ".$pendaftaran->nama."\n*Kode Pendaftaran :* ".$pendaftaran->code."\n*Jalur Pilihan :* ".$pendaftaran->jalur->nama.$tekswa,
            ]
        ];
        $kirim = Http::withHeaders([
            'Authorization' => 'Bearer EAASf8mQYtswBAOHMj6vFpB4cbRUi1Mga2vrfxigB5o4vyAA9ikaHNZA3UdFLgP5RGMLzZBBZAtkzC2dLljJ59qfdXMNUPZBvK3rE2R0yzHbZA3MWsiHyG0MiqsZBFSR6G8LQaRfFd7994PhPNtoJHGX6WDi77WUIWesqYu4vFg9v1o1c8yefaj'
        ])
        ->withBody(json_encode($data), 'application/json')
        ->post('https://graph.facebook.com/v17.0/113467801775666/messages');

        $check = WhatsAppChats::select("*")
            ->where('contacts_wa_id', $format_number)
            ->first();
        $save = new WhatsAppChatDetails();
        if($check) {
            $save->whatsapp_chat_id = $check->id;
            $check->updated_at = date("Y-m-d H:i:s");
            $check->save();
            $save->type = "out";
            $save->messages_id = $kirim['messages'][0]['id'];
            $save->messages_timestamp = strtotime(Date("Y-m-d H:i:s"));
            $save->messages_type = "text";
            $save->messages_text_body = $data['text']['body'];
            $save->save();
        }
        return $kirim;
    }
...
function WhatsappFacebookNoReplay($nomor) {
    $number = preg_replace( '/[^0-9]/', '', $nomor );
    if (substr($number, 0,2) != '62') {
        $no_wa = $number;
        $regional = 62;
    ...
    $whatsapp = Whatsapp::select("*")->where("bagian","=","Registrasi")->get();
    $tekswa = "";
    if($whatsapp){
    ...
    $data = [
        ...
        "type" => "text",
        "text" => [
            "body" => "Terima kasih telah menghubungi PMB Universitas Pancasila. Kami ingin memberitahukan bahwa pesan WhatsApp ini terkirim secra otomatis melalui sistem. Informasi terkait Pendaftaran Mahasiswa Baru, silakan menghubungi Admin PMB Universitas Pancasila dibawah ini :".$tekswa,
        ]
    ];
    $kirim = Http::withHeaders([
        'Authorization' => 'Bearer EAASf8mQYtswBAOHMj6vFpB4cbRUi1Mga2vrfxigB5o4vyAA9ikaHNZA3UdFLgP5RGMLzZBBZAtkzC2dLljJ59qfdXMNUPZBvK3rE2R0yzHbZA3MWsiHyG0MiqsZBFSR6G8LQaRfFd7994PhPNtoJHGX6WDi77WUIWesqYu4vFg9v1o1c8yefaj'
...
function WhatsappFacebookTemplatePendaftaran($nomor) {
    $number = preg_replace( '/[^0-9]/', '', $nomor );
    if (substr($number, 0,2) != '62') {
        $getnumber = substr($number, 1);
        $regional = 62;
...
function WhatsappFacebooKirimPesan($nomor, $chat) {
    $number = preg_replace( '/[^0-9]/', '', $nomor );
    if (substr($number, 0,2) != '62') {
        $getnumber = substr($number, 1);
        $regional = 62;
...
function findArrayObject($find, $array, $field){
...
function findArrayObjectNew($find, $array){
    foreach($array as $val){
        if($find == $val->code){
            return $val;
        }
...
function generateRandomCode() {
    ...
    $randomSeed = $timestamp % 1000000;
    $randomValue = mt_rand(0, 999999);
    $code = strtoupper(base_convert($randomSeed + $randomValue, 10, 36));
