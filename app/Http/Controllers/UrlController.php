...
class UrlController extends Controller
{
    ...
    public function __construct(
        ...
        $this->middleware('permission:settings-url-view',['only' => ['index','findById']]);
        $this->middleware('permission:settings-url-update',['only' => 'store']);
        $this->middleware('permission:settings-url-create',['only' => 'store']);
        $this->middleware('permission:settings-url-restore',['only' => 'restore']);
        $this->middleware('permission:settings-url-delete',['only' => 'remove']);
    }
    ...
    public function permission(Request $request, $id = null) {
        DB::beginTransaction();
        try {
            $data = $this->repository->permission($request, $id);
            DB::commit();
            $msg = 'succes saving data';
            if ($id) $msg = 'success update data';
            return H_apiResponse($data, $msg);
